...
def calculate_total_team_scores_sport(data):
    ...
    for date in data['Date'].unique():
        # Filter the rows for the current date and only keep rows with 'abs_activity' or 'rel_activity'
        date_data = data[(data['Date'] == date) & (data['Score_typ'].isin(['abs_activity', 'rel_activity']))]
...
def calculate_average_scores(fulldata):
    # Filter the relevant rows for abs_activity and rel_activity
    activity_data = fulldata[fulldata['Score_typ'].isin(['abs_activity', 'rel_activity'])]
    ...
    for date, group in activity_data.groupby('Date'):
        ...
        for col in participant_columns:
            ...
            rel_score = group.loc[group['Score_typ'] == 'rel_activity', col].sum()
...
def display_sport(full_data):
    ...
    sport_team_data = calculate_total_team_scores_sport(full_data)  # Get the total scores
    average_sport_scores_per_participant = calculate_average_scores(full_data)
